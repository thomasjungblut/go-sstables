...
func (r *FileReader) ReadNext() ([]byte, error) {
	...
	if r.header.fileVersion == Version1 {
		...
		recordHeaderPooled := r.bufferPool.Get(RecordHeaderV4MaxSizeBytes)
		defer r.bufferPool.Put(recordHeaderPooled)

		payloadSizeUncompressed, payloadSizeCompressed, recordNil, err := readRecordHeaderV4(r.reader, recordHeaderPooled)
		if err != nil {
...
func (r *FileReader) SkipNext() error {
	...
	if r.header.fileVersion == Version1 {
		...
		payloadSizeUncompressed, payloadSizeCompressed, _, err := readRecordHeaderV4(r.reader, recordHeaderPooled)
		if err != nil {
